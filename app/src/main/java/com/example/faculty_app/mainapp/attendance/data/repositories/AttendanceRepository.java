...
public class AttendanceRepository {
    public static void fetchSessions(int classId, RepositoryCallback<SessionsResponse> callback) {
        AxisAttendance.getSessions(
                classId, new AxisCallback<SessionsResponse>() {
                    @Override
                    public void onResult(AxisResult<SessionsResponse> result) {
                        if (result instanceof AxisResult.Success) {
                            var data = ((AxisResult.Success<SessionsResponse>) result).getData();

                            callback.onResult(new RepositoryResult.Success<>(data));
                        }
                        else if (result instanceof AxisResult.Fail) {
                            var fail = (AxisResult.Fail<SessionsResponse>) result;
                            var code = fail.code;
                            callback.onResult(new RepositoryResult.Fail<>(new AttendanceException(
                                    code == null ?
                                    AttendanceExceptionCode.API_DISCONNECTED :
                                    AttendanceExceptionCode.API_EXCEPTION,
                                    fail.message,
                                    fail.throwable
                            )));
                        }
    ...
    public static void fetchSessionAttendance(int classSessionId,
                                              RepositoryCallback<SessionAttendanceResponse> callback) {
        AxisAttendance.getSessionAttendance(
                classSessionId, new AxisCallback<SessionAttendanceResponse>() {
                    @Override
                    public void onResult(AxisResult<SessionAttendanceResponse> result) {
                        if (result instanceof AxisResult.Success) {
                            var data =
                                    ((AxisResult.Success<SessionAttendanceResponse>) result).getData();

                            callback.onResult(new RepositoryResult.Success<>(data));
                        }
                        else if (result instanceof AxisResult.Fail) {
                            var fail = (AxisResult.Fail<SessionAttendanceResponse>) result;
                            var code = fail.code;
                            callback.onResult(new RepositoryResult.Fail<>(new AttendanceException(
                                    code == null ?
                                    AttendanceExceptionCode.API_DISCONNECTED :
    ...
    public static void fetchStudentAttendance(int enrollmentId,
        ...
        AxisAttendance.getStudentAttendance(
                enrollmentId, new AxisCallback<StudentAttendance>() {
                    @Override
